...
In this video, we create a debouncer circuit using a state machine and verify it using the Basys3's 7-segment display. The buttons on the Basys3 are analog and susceptible to noise. A debouncer can be used to verify the input from our button so that we only count a press every time the button is pushed. A state machine is a structure we can use to organize our logic. 
